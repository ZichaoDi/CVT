...
global GRAPH_N_OLD GRAPH_INDEX_OLD miu ipivot lambdatn lambdaind  shiftc
global Hess_norm_H % norm of coarse-grid Hessian
global test_v_in_fmincon v_after_r bind spec_bd  vvl coalow vvu bindH
%----------------------------------------------------------------------
...
if (current_n <= nmin);
    ...
    if (step_bnd == 0);
        if (bounds);
            ...
            [low1,up1] = set_bounds(j,res_prob);
            nit = nit_solve; [v,F,G,ierror]  = tn  (v0,myfun,low1,up1,nit);
        end;
    else
        [low1,up1] = set_bounds(j,res_prob);
        nit = nit_solve; [v,F,G, ierror] = tn (v0,myfun,low1,up1,nit);
    ...
    if (step_bnd == 0);
        if (bounds);
            ...
            [low1,up1] =set_bounds(j,res_prob);
            nit = 1; [v,F,G,ierror,eig_val]  = tn  (v0,myfun,low1,up1,nit);
        end;
    else
        [low1,up1] = set_bounds(j,res_prob);
        nit = 1; [v,F,G,ierror,eig_val]  = tn(v0,myfun,low1,up1,nit);
    ...
    if (kk > 0)
        ...
        accrcy = 128*eps;
        vnorm  = norm(v, 'inf');
        He = gtims (e, v, G_before_recursion_reg, ipivot,accrcy, vnorm, @sfun,miu);
...
%     end
    ...
    if (testd<0)
        ...
        if(bounds)   %%%%%%%%%%%%%This line search is based on merit function
    ...
    if (step_bnd == 0);
        if (bounds);
            ...
            [low1,up1] = set_bounds(j,res_prob);;
            nit = 1; [v,F,G,ierror]  = tn  (v,myfun,low1,up1,nit);
        end;
    else
        [low1,up1] = set_bounds(j,res_prob);;
        nit = 1; [v,F,G,ierror]  = tn(v,myfun,low1,up1,nit);
