...
if (current_n <= nmin);
    ...
    if (step_bnd == 0);
        ...
        if (bounds);
            nit = nit_solve; [v,F,G,ierror]  = tnbcm1(v0,myfun,v_low,v_up,nit);
        else
            nit = nit_solve; [v,F,G,ierror]  = tnm1  (v0,myfun,nit);
        end;
        
    else
        [low1,up1] = get_bnd (v0,step_bnd);
        nit = nit_solve; [v,F,G,ierror]  = tnbcm1(v0,myfun,low1,up1,nit);
    end
else
    %--------------------------------------------------
    ...
    if (step_bnd == 0);
        ...
        if (bounds);
            ...
            nit = 1; [v,F,G,ierror,eig_val]  = tnbcm1(v0,myfun,v_low,v_up,nit);
        else
            nit = 1; [v,F,G,ierror,eig_val]  = tnm1  (v0,myfun,nit);
        end;
        
    else
        [low1,up1] = get_bnd (v0,step_bnd);
        nit = 1; [v,F,G,ierror,eig_val]  = tnbcm1(v0,myfun,low1,up1,nit);
...
%     end;
    ...
    %--------------------------------------------------
    [e2, pred]  = mgrid1(current_v,fnl2,1,step_bnd1);
   % e2=sort(e2);
    %v=sort(v);
...
%                 figure(15);plot(vstar-v,'b.-'); title('Step to solution')
%                 tt = linspace(0,1,n+2)'; tt = tt(2:end-1);
%                 figure(16); plot(tt,vstar-v,'b',tt,2*e,'r'); title('Step (red), Direction (blue)')
%                 
...
%    disp(T)
    ...
    if (step_bnd == 0);
        if (bounds);
            nit = 1; [v,F,G,ierror]  = tnbcm1(v,myfun,v_low,v_up,nit);
        else
            nit = 1; [v,F,G,ierror]  = tnm1  (v,myfun,nit);
        end;
    else
        [low1,up1] = get_bnd (v,step_bnd);
        nit = 1; [v,F,G,ierror]  = tnbcm1(v,myfun,low1,up1,nit);
    end;
end
