function [XY2,B,globalind] = coarsen3_mlevel(XY,XYcur)
global bdry
...
leftout = setdiff(1:nall,taken);
for jj=1:length(leftout)
    for i=1:step-1 diff(i) = distance(XY(:,leftout(jj)),XY2(:,i)); end
    [m,ii]=min(diff);
    B(leftout(jj),ii)=1;    
...
B = B(1:nall,finalindex); % relevant part of the interpolation matrix
%B(i,j) = coef of i-th node wrt globalind(j)-th coarse node
