...
ffOUT = [fileOUT,'.txt'];
figOUT = [fileOUT,'.fig']
arrayOUT = ['C:\DOC\MASHA\Du\multigrid\1d\may04\constResults\NrhoTableNewNorm.txt'];
...
nu = 2; %number of relaxations on coarsest level
...
%------------------------------------------------
while (outer<=Ncycles)&(abs(current_error)>eps)
...
errAll(pp) = norm(data - dataPred)/sqrt(N);
 ...
 if (outer==1)&&(ss==NGridStep)
     error0 = norm(data-dataPred)/sqrt(N);
...
errorVE(outer) = engy(pp);
errorE = abs(log(errorVE));
errorV(outer) = errAll(pp);
error = abs(log(errorV));

if (outer>1) 
    convergenceE(outer)=errorE(outer)/errorE(outer-1); 
    convergence(outer)=error(outer)/error(outer-1); 
end


if (outer>1) current_error = (engy(pp-1)-engy(pp))/engy0;
else
    current_error = (engy0-engy(pp))/engy0;    
end 

if (outer>2) 
     rrE(outer) = (errorVE(outer)-errorVE(outer-1))/(errorVE(outer-1)-errorVE(outer-2));
     rr(outer) = (errorV(outer)-errorV(outer-1))/(errorV(outer-1)-errorV(outer-2));
else
     rrE(outer)=1;
     rr(outer)=1;
end     
