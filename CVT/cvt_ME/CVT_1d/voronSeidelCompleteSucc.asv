function zzz = voronSeidelCompleteSucc(N,eps,NGridStep,Ncycles,mu1,mu2,mu0,fileIN, fileOUT,handles,hObject)
...
data = double(partRelaxNosymSucc(data,Jac,NGridStep-ss));
...
for i=1:nu
 rel=i   
 T = data;
 data  = double(partRelaxNosymSucc(data,Jac,NGridStep-1)); 
 if (norm(data)<=eps) 
    data
...
Jac;
for i=1:mu0
 data = double(partRelaxNosymSucc(data,Jac,NGridStep-2));
